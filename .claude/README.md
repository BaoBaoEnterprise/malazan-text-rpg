...
.claude/
  ...
  skills/                 Procedures Claude loads on demand.
    add-enemy/SKILL.md      End-to-end: add a foe, then verify it.
  ...
  commands/               Slash commands you can type.
    ...
    add-enemy.md            /add-enemy <idea> — scaffold a new encounter.
  hooks/
...
## How the pieces fit
- **Agents** are for delegation — "add three new foes" → `content-designer`; "add a stealth
  mechanic" → `engine-dev`; "check the new lore" → `lore-guardian`.
- **Skills** are step-by-step procedures Claude pulls in automatically when relevant (or you
  can name them).
- **Commands** are things *you* type: `/serve`, `/add-enemy a Jaghut weak to otataral`.
